...
function main() {
    ...
    // If package trigger set a temp save-path, run report immediately without waiting for UI button click.
    var packageTriggerPathFile = new File(Folder.temp.fsName + "/MacmillanFontReportSavePath.txt");
    var isPackageTriggeredRun = packageTriggerPathFile.exists;
    if (isPackageTriggeredRun) {
        var autoData = scanFonts(doc);
        var autoReport = exportCSV(doc, autoData);
        if (autoReport && autoReport.fileObj && autoReport.fileObj.exists) {
            try {
                autoReport.fileObj.execute();
            } catch (e) {
                alert("Report saved, but could not be opened automatically.\n" + autoReport.fileObj.fsName);
            }
        }
        return;
    }
